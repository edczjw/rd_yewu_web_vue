<template>
  <div>
    ...
    <div class="outpadding">
       <!-- <p class="xiangqingtitle"><i class="el-icon-location-outline"></i>智慧万商</p> -->

      <el-tabs v-model="tabname" type="card">
        <!-- 有两大板块，分别来自不同的页面 -->
        <el-tab-pane label="基本信息" name="userbasewanshang"></el-tab-pane>
        <el-tab-pane label="影像信息" name="userpics"></el-tab-pane>
      </el-tabs>
      <userbasewanshang v-if="tabname=='userbasewanshang'"></userbasewanshang>
      <userpics v-if="tabname=='userpics'"></userpics>
    </div>
  </div>
</template>
<script>
import userbasewanshang from "./userbasewanshang.vue";
...
export default {
  data() {
    return {
      ...
      detail: {},
      tabname: "userbasewanshang",//默认打开的是基本信息
    };
  },
  mounted() {
    window.onresize = () => {
